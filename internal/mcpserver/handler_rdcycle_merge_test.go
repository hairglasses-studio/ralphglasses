...
func TestHandleCycleMerge_HappyPath(t *testing.T) {
	...
	if err := exec.Command("git", "-C", repo, "branch", "-M", "main").Run(); err != nil {
		t.Fatalf("git command failed: %v", err)
	}

	// branch 1
	if err := exec.Command("git", "-C", repo, "checkout", "-b", "branch1").Run(); err != nil {
		t.Fatalf("git command failed: %v", err)
	}
	...
	if err := exec.Command("git", "-C", repo, "commit", "-m", "branch1").Run(); err != nil {
		t.Fatalf("git command failed: %v", err)
	}

	// branch 2
	if err := exec.Command("git", "-C", repo, "checkout", "main").Run(); err != nil {
		t.Fatalf("git command failed: %v", err)
	}
	if err := exec.Command("git", "-C", repo, "checkout", "-b", "branch2").Run(); err != nil {
		t.Fatalf("git command failed: %v", err)
	}
	...
	if err := exec.Command("git", "-C", repo, "commit", "-m", "branch2").Run(); err != nil {
		t.Fatalf("git command failed: %v", err)
	}
	...
	if err := exec.Command("git", "-C", repo, "worktree", "add", wt1, "branch1").Run(); err != nil {
		t.Fatalf("git command failed: %v", err)
	}
	if err := exec.Command("git", "-C", repo, "worktree", "add", wt2, "branch2").Run(); err != nil {
		t.Fatalf("git command failed: %v", err)
	}
