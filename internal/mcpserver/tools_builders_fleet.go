	...
	return ToolGroup{
		...
		Tools: []ToolEntry{
			...
			{mcp.NewTool("ralphglasses_fleet_analytics",
				mcp.WithDescription("Cost breakdown by provider/repo/time-period with trend analysis. Requires a fleet coordinator started via `ralphglasses serve --coordinator` and an MCP client configured with `RALPH_FLEET_URL`."),
			...
			{mcp.NewTool("ralphglasses_fleet_submit",
				mcp.WithDescription("Submit work for `repo` with `prompt` to the distributed fleet queue. Requires a fleet coordinator started via `ralphglasses serve --coordinator` and an MCP client configured with `RALPH_FLEET_URL`."),
			...
			{mcp.NewTool("ralphglasses_fleet_budget",
				mcp.WithDescription("View or set the fleet-wide budget. Shows spent, remaining, and active work. Requires a fleet coordinator started via `ralphglasses serve --coordinator` and an MCP client configured with `RALPH_FLEET_URL`."),
			...
			{mcp.NewTool("ralphglasses_fleet_workers",
				mcp.WithDescription("List registered fleet workers with status, capacity, and spend. Optionally pause, resume, or drain a worker. Requires a fleet coordinator started via `ralphglasses serve --coordinator` and an MCP client configured with `RALPH_FLEET_URL`."),
			...
			{mcp.NewTool("ralphglasses_fleet_runtime",
				mcp.WithDescription("Start, stop, restart, inspect, or auto-discover fleet runtime mode parity for the CLI serve command"),
			...
			{mcp.NewTool("ralphglasses_marathon",
				mcp.WithDescription("Start, resume, inspect, or stop the CLI marathon workflow"),
	...
	return ToolGroup{
		...
		Tools: []ToolEntry{
			{mcp.NewTool("ralphglasses_blackboard_query",
				mcp.WithDescription("Query blackboard entries by namespace for fleet worker coordination. Requires a fleet coordinator started via `ralphglasses serve --coordinator` and an MCP client configured with `RALPH_FLEET_URL`."),
			...
			{mcp.NewTool("ralphglasses_blackboard_put",
				mcp.WithDescription("Write an entry to the blackboard for fleet coordination. Requires a fleet coordinator started via `ralphglasses serve --coordinator` and an MCP client configured with `RALPH_FLEET_URL`."),
			...
			{mcp.NewTool("ralphglasses_a2a_offers",
				mcp.WithDescription("List open agent-to-agent task delegation offers. Requires a fleet coordinator started via `ralphglasses serve --coordinator` and an MCP client configured with `RALPH_FLEET_URL`."),
			), s.handleA2AOffers},
			{mcp.NewTool("ralphglasses_cost_forecast",
				mcp.WithDescription("Cost burn rate, anomaly detection, and budget exhaustion ETA. Requires a fleet coordinator started via `ralphglasses serve --coordinator` and an MCP client configured with `RALPH_FLEET_URL`."),
	...
	return ToolGroup{
		...
		Tools: []ToolEntry{
			...
			{mcp.NewTool("ralphglasses_repo_surface_audit",
				mcp.WithDescription("Audit repo instruction/config surfaces used by Codex, Claude, Gemini, and MCP clients"),
				mcp.WithString("repo", mcp.Required(), mcp.Description("Repo name")),
			...
			{mcp.NewTool("ralphglasses_worktree_list",
				mcp.WithDescription("List git worktrees for a repo with optional dirty/stale filtering"),
				mcp.WithString("repo", mcp.Required(), mcp.Description("Repo name")),
