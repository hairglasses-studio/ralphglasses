...
func TestHandleSessionHandoff_UsesRuntimeDefaultWhenSourceProviderEmpty(t *testing.T) {
	t.Parallel()
...
func TestHandleSessionHandoff_AllowsOllamaTarget(t *testing.T) {
	t.Parallel()
	...
	if captured.Provider != session.ProviderOllama {
		t.Fatalf("captured provider = %q, want %q", captured.Provider, session.ProviderOllama)
	}
	if captured.Model != session.ProviderDefaults(session.ProviderOllama) {
		t.Fatalf("captured model = %q, want %q", captured.Model, session.ProviderDefaults(session.ProviderOllama))
	}
}
