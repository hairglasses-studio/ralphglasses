	...
	return ToolGroup{
		...
		Tools: []ToolEntry{
			{mcp.NewTool("ralphglasses_session_launch",
				...
				mcp.WithString("provider", mcp.Description("LLM provider: omit/auto for runtime selection, or codex, claude, gemini, ollama, antigravity. Antigravity opens an external interactive handoff instead of a managed streaming session.")),
				mcp.WithString("model", mcp.Description("Model to use")),
				mcp.WithNumber("budget_usd", mcp.Description("Budget in USD — maximum spend for this session. Native for Claude/Ollama; externally enforced by ralphglasses for Codex, Gemini, and Antigravity.")),
				mcp.WithNumber("max_turns", mcp.Description("Maximum conversation turns. Native for Claude/Ollama only.")),
				mcp.WithString("agent", mcp.Description("Agent/subagent name. Native for Claude/Ollama only.")),
				mcp.WithString("allowed_tools", mcp.Description("Comma-separated allowed tools (e.g. Bash,Read,Edit). Supported by Claude, Ollama, and Gemini; unsupported by Codex and Antigravity.")),
				mcp.WithString("system_prompt", mcp.Description("Additional system prompt to append. Native for Claude/Ollama only; use GEMINI.md or AGENTS.md plus .agents/rules/.agents/workflows for Gemini, Codex, and Antigravity repo instructions.")),
				...
				mcp.WithString("target_provider", mcp.Description("Target LLM provider for prompt enhancement: claude, gemini, openai, or ollama (ollama uses the openai-compatible prompt structure style; defaults to the session provider lane)")),
			...
			{mcp.NewTool("ralphglasses_session_list",
				...
				mcp.WithString("tenant_id", mcp.Description("Workspace tenant ID (default: _default)")),
				mcp.WithString("provider", mcp.Description("Filter by provider: claude, gemini, codex, ollama, antigravity (omit for all)")),
			...
			{mcp.NewTool("ralphglasses_session_resume",
				...
				mcp.WithString("provider", mcp.Description("LLM provider: omit/auto for runtime selection, or codex, claude, gemini, ollama. Antigravity is launch-only and cannot be resumed through ralphglasses.")),
			...
			{mcp.NewTool("ralphglasses_session_handoff",
				...
				mcp.WithString("target_provider", mcp.Description("Target provider: omit/auto for runtime selection, or claude, gemini, codex, ollama, antigravity (default: same as source)")),
	...
	return ToolGroup{
		...
		Tools: []ToolEntry{
			{mcp.NewTool("ralphglasses_loop_start",
				...
				mcp.WithString("planner_provider", mcp.Description("Planner provider: claude, gemini, codex, ollama (default: codex)")),
				mcp.WithString("worker_provider", mcp.Description("Worker provider: claude, gemini, codex, ollama (default: codex)")),
				mcp.WithString("verifier_provider", mcp.Description("Verifier provider: claude, gemini, codex, ollama (default: codex)")),
			...
			{mcp.NewTool("ralphglasses_self_improve",
				...
				mcp.WithString("planner_provider", mcp.Description("Planner provider (claude, gemini, codex, ollama)")),
				mcp.WithString("worker_provider", mcp.Description("Worker provider (claude, gemini, codex, ollama)")),
				mcp.WithString("trace_level", mcp.Description("Trace verbosity: none, summary (default), verbose")),
			), s.handleSelfImprove},
	...
	return ToolGroup{
		...
		Tools: []ToolEntry{
			{mcp.NewTool("ralphglasses_prompt_analyze",
				...
				mcp.WithString("target_provider", mcp.Description("Target model provider for scoring suggestions: openai or ollama (default openai-style), claude, gemini")),
			...
			{mcp.NewTool("ralphglasses_prompt_enhance",
				...
				mcp.WithString("task_type", mcp.Description("Override auto-detection: code, troubleshooting, analysis, creative, workflow, general")),
				mcp.WithString("mode", mcp.Description("Enhancement mode: local (default, deterministic), llm (Claude/Gemini/OpenAI or local Ollama-compatible API), auto (try LLM, fallback to local)")),
				mcp.WithString("repo", mcp.Description("Repo name to load .prompt-improver.yaml config from")),
				mcp.WithString("target_provider", mcp.Description("Target model provider — controls structure style and scoring: openai or ollama (default openai-style), claude, gemini")),
				mcp.WithString("trace_level", mcp.Description("Trace verbosity: none, summary (default), verbose")),
			...
			{mcp.NewTool("ralphglasses_prompt_improve",
				...
				mcp.WithString("provider", mcp.Description("LLM provider for improvement: openai (default), ollama (local OpenAI-compatible lane), claude, gemini")),
			...
			{mcp.NewTool("ralphglasses_prompt_ab_test",
				...
				mcp.WithString("target_provider", mcp.Description("Target provider for scoring: openai or ollama (default openai-style), claude, gemini")),
