...
#   hw-detect.sh --wayland-only        # Run as root, writes Sway + Hyprland configs (skip Xorg)
...
COMPOSITOR_CONFIGS_WRITTEN="none"
...
"
    ...
    log "--- Hyprland Configuration ---"

    HYPR_CONF_DIR="/etc/hyprland"
    HYPR_MONITORS="${HYPR_CONF_DIR}/monitors.conf"

    HYPR_CONTENT="# Generated by hw-detect.sh --wayland-only
# Hyprland monitor configuration for detected GPUs
# Customize positions to match your physical monitor layout

"
    if [[ -n "$RTX4090_BUS" ]]; then
        HYPR_CONTENT+="# RTX 4090 outputs (PCI $RTX4090_BUS)
# monitor = DP-1, 3840x2160, 0x0, 1.0
# monitor = DP-2, 3840x2160, 3840x0, 1.0
# monitor = DP-3, 3840x2160, 7680x0, 1.0
# monitor = HDMI-A-1, 3840x2160, 11520x0, 1.0
"
    fi
    ...
    if [[ -n "$AMD_IGPU_BUS" ]]; then
        HYPR_CONTENT+="
# AMD iGPU outputs (PCI $AMD_IGPU_BUS) — secondary/overflow
# monitor = HDMI-A-2, 1920x1080, 15360x0, 1.0
"
    fi

    # Set AQ_DRM_DEVICES for GPU ordering (Hyprland equivalent of WLR_DRM_DEVICES)
    if [[ -n "$RTX4090_BUS" ]] && [[ -n "$AMD_IGPU_BUS" ]]; then
        HYPR_CONTENT+="
# Dual GPU: NVIDIA primary, AMD secondary
# Set AQ_DRM_DEVICES in environment.d or hyprland.conf:
# env = AQ_DRM_DEVICES,/dev/dri/card0:/dev/dri/card1
"
    fi

    write_file "$HYPR_MONITORS" "$HYPR_CONTENT"
    log "Wrote Hyprland monitor config to $HYPR_MONITORS"

    COMPOSITOR_CONFIGS_WRITTEN="sway,hyprland"
fi
...
COMPOSITOR_CONFIGS_WRITTEN=${COMPOSITOR_CONFIGS_WRITTEN:-none}
"
